			...
			mockRows: func(mock *MockDB) {
				rows := &MockRows{
					data: [][]interface{}{
						{
							1, "Иван", "Иванов", true, 180,
							time.Date(1990, 1, 1, 0, 0, 0, 0, time.UTC), "Описание",
...
func TestGetProfilesByUserId(t *testing.T) {
	...
	// Настройка мока для первого вызова (profile_id = 1)
	mockDB.On("Query", mock.Anything, expectedSQL, []interface{}{1}).
		Return(&MockRows{
			...
			// 	"profile_id", "firstname", "lastname", "is_male", "height",
			// 	"birthday", "description", "country", "city", "district",
			// 	"liked_by_profile_id", "avatar", "interest",
			// 	"preference_description", "preference_value",
			// },
		}, nil)

	// Настройка мока для второго вызова (profile_id = 2)
